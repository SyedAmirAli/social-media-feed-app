...
const THEME_STORAGE_KEY = "buddy_script_theme";
...
export default function Theme() {
    ...
    useEffect(() => {
        try {
            ...
            setIsDark(localStorage.getItem(THEME_STORAGE_KEY) === "dark");
        } catch {
            /* ignore */
        }
        setReady(true);
    ...
    useEffect(() => {
        ...
        layout?.classList.toggle("_dark_wrapper", isDark);
    ...
    // Own the click in capture phase so custom.js cannot double-toggle.
    useEffect(() => {
        const onClick = (event: MouseEvent) => {
            const target = event.target as Element | null;
            if (!target?.closest?.("._layout_swithing_btn_link")) return;
